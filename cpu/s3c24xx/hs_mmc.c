...
static int wait_for_cmd_done (void)
{
	...
	for (i = 0; i < 0x20000000; i++) {
		...
		// by crazyboy, dbg("  HM_NORINTSTS: %04x\n", n_int);
...
static int set_sd_ocr (void)
{
	...
	for (i = 0; i < 100; i++) {
		ocr |= 0x40000000; /* by crazyboy_edchoi, for SDHC, set HCS bit of OCR */
		issue_command(MMC_APP_CMD, 0x0, 0, MMC_RSP_R1);
...
#if 1	/* by crazyboy_edchoi, for SDHC */
	udelay(2000);
	while (issue_command(MMC_SEND_EXT_CSD, 0x1AA, 0, MMC_RSP_R7) == 0);
	dbg("CMD8 RSP = %0x%x\n", s3c_hsmmc_readl(HM_RSPREG0));
#endif

	ocr_check = 1;

#if 1	/* bu crazyboy_edchoi, check SD first */
	if (set_sd_ocr()) {
		mmc_card = 0;
